...
const StudentProfile: React.FC<StudentProfileProps> = ({
  ...
  const [isProfileVerified, setIsProfileVerified] = useState<boolean>(() => {
    const v = localStorage.getItem(`cpms_profile_verified_${userId}`) || localStorage.getItem(`cpms_profile_verified_${userEmailKey}`) || localStorage.getItem("cpms_profile_verified_global");
    const st = localStorage.getItem(`cpms_verification_status_${userId}`) || localStorage.getItem(`cpms_verification_status_${userEmailKey}`) || localStorage.getItem("cpms_verification_status_global");
    return v === "true" && st !== "pending" && st !== "rejected";
  });
  const [verificationStatus, setVerificationStatus] = useState<string>(() => {
    return localStorage.getItem(`cpms_verification_status_${userId}`) || localStorage.getItem(`cpms_verification_status_${userEmailKey}`) || (isProfileVerified ? "verified" : "pending");
  });
  const [rejectionReason, setRejectionReason] = useState<string>(() => {
    return localStorage.getItem(`cpms_rejection_reason_${userId}`) || localStorage.getItem(`cpms_rejection_reason_${userEmailKey}`) || "";
  });
  const [pendingChangesList, setPendingChangesList] = useState<any[]>(() => {
    try {
      const saved = localStorage.getItem(`cpms_pending_diff_${userId}`) || localStorage.getItem(`cpms_pending_diff_${userEmailKey}`) || localStorage.getItem("cpms_pending_diff_global");
  ...
  const [location, setLocation] = useState(cachedProfileData?.personal?.location || "");
  const [gender, setGender] = useState(cachedProfileData?.personal?.gender || "");
  const [dob, setDob] = useState(cachedProfileData?.personal?.dob || "");
  ...
  useEffect(() => {
    ...
    // Lightweight status checker that ONLY checks verification boolean and syncs cleanly
    const checkVerificationOnly = async () => {
      try {
        const lookupKey = userId || (user?.email || "").toLowerCase().trim();
        if (!lookupKey) return;
        const res = await fetch(`${API_BASE_URL}/api/student/profile/${encodeURIComponent(lookupKey)}?email=${encodeURIComponent((user?.email || "").toLowerCase().trim())}`);
        if (res.ok) {
          const data = await res.json();
          if (data) {
            const isVer = Boolean(
    ...
    const handleSync = () => {
      ...
      const updatedCache = getCachedProfile();
      if (updatedCache) {
        if (updatedCache.personal?.email) setEmail(updatedCache.personal.email);
        if (updatedCache.personal?.fullName) setFullName(updatedCache.personal.fullName);
        if (updatedCache.personal?.phone) setPhone(updatedCache.personal.phone);
        if (updatedCache.personal?.registerNumber) setRegisterNumber(updatedCache.personal.registerNumber);
        if (updatedCache.personal?.department) setDepartment(updatedCache.personal.department);
      }
    };
    ...
    return () => {
      ...
      window.removeEventListener("cpms_verification_updated", handleSync);
      window.removeEventListener("cpms_profile_updated", handleSync);
    ...
    try {
      ...
      const profilePayload = {
        ...
        academic: {
          ...
          pgProgram: pgProgram.trim(),
          pgSpecialization: pgSpecialization.trim(),
          pgGradYear: pgGradYear !== "" ? Number(pgGradYear) : "",
          pgCgpa: pgCgpa !== "" ? Number(pgCgpa) : "",
          pgSemester
        },
        professional: {
      ...
      // Compare against currently approved baseline data to accurately record ONLY modified fields
      const cachedApproved = getCachedProfile();
      const baseline = baseProfileRef.current || cachedApproved;
      const changedList: Array<{ field: string; label: string; oldVal: string; newVal: string }> = [];
      const addDiff = (field: string, label: string, oldV: any, newV: any) => {
        ...
        if (normNew !== normOld) {
          changedList.push({ field, label, oldVal: normOld || "N/A", newVal: normNew || "N/A" });
        }
      };

      addDiff("fullName", "Full Name", baseline?.personal?.fullName || baseline?.user?.name || cachedApproved?.personal?.fullName || user?.name, fullName);
      addDiff("phone", "Phone Number", baseline?.personal?.phone || cachedApproved?.personal?.phone, phone);
      addDiff("email", "Email Address", baseline?.personal?.email || baseline?.user?.email || cachedApproved?.personal?.email || user?.email, email);
      addDiff("department", "Department", baseline?.personal?.department || cachedApproved?.personal?.department, department);
      addDiff("registerNumber", "Register Number", baseline?.personal?.registerNumber || cachedApproved?.personal?.registerNumber, registerNumber);
      addDiff("location", "Location", baseline?.personal?.location ?? cachedApproved?.personal?.location, location);
      addDiff("gender", "Gender", baseline?.personal?.gender ?? cachedApproved?.personal?.gender, gender);
      addDiff("dob", "Date of Birth", baseline?.personal?.dob ?? cachedApproved?.personal?.dob, dob);

      addDiff("tenthPercentage", "10th Percentage", baseline?.academic?.tenthPercentage ?? cachedApproved?.academic?.tenthPercentage, tenthPercentage);
      addDiff("twelfthPercentage", "12th Percentage", baseline?.academic?.twelfthPercentage ?? cachedApproved?.academic?.twelfthPercentage, twelfthPercentage);
      addDiff("schoolName", "School Name", baseline?.academic?.schoolName ?? cachedApproved?.academic?.schoolName, schoolName);
      addDiff("diplomaInstitution", "Diploma Institution", baseline?.academic?.diplomaInstitution ?? cachedApproved?.academic?.diplomaInstitution, diplomaInstitution);
      addDiff("diplomaSpecialization", "Diploma Specialization", baseline?.academic?.diplomaSpecialization ?? cachedApproved?.academic?.diplomaSpecialization, diplomaSpecialization);
      addDiff("ugInstitution", "UG Institution", baseline?.academic?.ugInstitution ?? cachedApproved?.academic?.ugInstitution, ugInstitution);
      addDiff("appNumber", "Application Number", baseline?.academic?.appNumber ?? cachedApproved?.academic?.appNumber, appNumber);
      addDiff("ugProgram", "UG Program", baseline?.academic?.ugProgram ?? cachedApproved?.academic?.ugProgram, ugProgram);
      addDiff("ugSpecialization", "UG Specialization", baseline?.academic?.ugSpecialization ?? cachedApproved?.academic?.ugSpecialization, ugSpecialization);
      addDiff("currentSemester", "Current Semester", baseline?.academic?.currentSemester ?? cachedApproved?.academic?.currentSemester, currentSemester);
      addDiff("graduationYear", "Graduation Year", baseline?.academic?.graduationYear ?? cachedApproved?.academic?.graduationYear, graduationYear);
      addDiff("cgpa", "CGPA", baseline?.academic?.cgpa ?? cachedApproved?.academic?.cgpa, cgpa);
      addDiff("backlogs", "Active Backlogs", baseline?.academic?.backlogs ?? cachedApproved?.academic?.backlogs, backlogs);
      addDiff("backlogHistory", "Backlog History", baseline?.academic?.backlogHistory ?? cachedApproved?.academic?.backlogHistory, backlogHistory);
      addDiff("pgInstitution", "PG Institution", baseline?.academic?.pgInstitution ?? cachedApproved?.academic?.pgInstitution, pgInstitution);
      addDiff("pgProgram", "PG Program", baseline?.academic?.pgProgram ?? cachedApproved?.academic?.pgProgram, pgProgram);
      addDiff("pgSpecialization", "PG Specialization", baseline?.academic?.pgSpecialization ?? cachedApproved?.academic?.pgSpecialization, pgSpecialization);
      addDiff("pgGradYear", "PG Graduation Year", baseline?.academic?.pgGradYear ?? cachedApproved?.academic?.pgGradYear, pgGradYear);
      addDiff("pgCgpa", "PG CGPA", baseline?.academic?.pgCgpa ?? cachedApproved?.academic?.pgCgpa, pgCgpa);
      addDiff("pgSemester", "PG Semester", baseline?.academic?.pgSemester ?? cachedApproved?.academic?.pgSemester, pgSemester);

      const oldSkills = Array.isArray(baseline?.professional?.skills) ? baseline.professional.skills.join(", ") : (baseline?.professional?.skills || "");
      const oldCerts = Array.isArray(baseline?.professional?.certifications) ? baseline.professional.certifications.join(", ") : (baseline?.professional?.certifications || "");
      const oldProjects = Array.isArray(baseline?.professional?.projects) ? baseline.professional.projects.join(", ") : (baseline?.professional?.projects || "");
      const oldInternships = Array.isArray(baseline?.professional?.internships) ? baseline.professional.internships.join(", ") : (baseline?.professional?.internships || "");
      addDiff("skills", "Skills", oldSkills, skills);
      addDiff("certifications", "Certifications", oldCerts, certifications);
      addDiff("projects", "Projects", oldProjects, projects);
      addDiff("internships", "Internships", oldInternships, internship);

      if (resumeFile) {
        changedList.push({ field: "resume", label: "Resume File", oldVal: baseline?.professional?.resumeName || cachedApproved?.professional?.resumeName || "No Resume", newVal: resumeFile.name });
      }

      const editedFieldsList = changedList.map(c => c.field);
      setPendingChangesList(changedList);
      ...
      if (userId) {
        ...
        localStorage.setItem(`cpms_pending_fields_${userId}`, JSON.stringify(editedFieldsList));
      ...
      if (userKey) {
        ...
        localStorage.setItem(`cpms_pending_fields_${userKey}`, JSON.stringify(editedFieldsList));
      ...
      localStorage.setItem("cpms_pending_fields_global", JSON.stringify(editedFieldsList));
      ...
      <div style={{ backgroundColor: "#FFFFFF", borderRadius: "16px", border: "1px solid #E2E8F0", padding: "24px 28px", boxShadow: "0 2px 8px rgba(0,0,0,0.03)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "24px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "22px", flexWrap: "wrap", flex: "1 1 500px" }}>
          ...
          <div style={{ position: "relative", width: "90px", height: "90px", flexShrink: 0 }}>
            <label style={{ width: "90px", height: "90px", borderRadius: "50%", border: "3px solid #4F46E5", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "#EFF6FF", cursor: "pointer", position: "relative", boxShadow: "0 4px 12px rgba(11, 61, 145, 0.15)" }} title="Click to upload profile photo">
              <input
                type="file"
                accept="image/*"
              ...
              {profileImage ? (
                ...
                <div style={{ width: "100%", height: "100%", backgroundColor: "#E6EEFC", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <svg width="46" height="46" viewBox="0 0 24 24" fill="#4F46E5">
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "12px", minWidth: "240px", backgroundColor: "#F8FAFC", padding: "16px 18px", borderRadius: "12px", border: "1px solid #E2E8F0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "13px", fontWeight: 800, color: "#4F46E5" }}>{completionPercentage}%</span>
          </div>

          {/* Progress Bar */}
          <div style={{ width: "100%", height: "8px", backgroundColor: "#E2E8F0", borderRadius: "10px", overflow: "hidden" }}>
            <div style={{ width: `${completionPercentage}%`, height: "100%", backgroundColor: "#4F46E5", borderRadius: "10px", transition: "width 0.3s ease" }}></div>
          </div>
