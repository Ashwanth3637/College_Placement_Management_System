...
export const ReportsAnalyticsManagement: React.FC = () => {
  ...
  return (
    <div style={{
      ...
      {/* Mathematically Consistent 6-KPI Metric Grid Bar Matching Dashboard Card Style */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: "14px", marginBottom: "20px" }}>
        {[
          { label: "TOTAL STUDENTS", value: summary.totalStudents, sub: "Registered candidates", color: "#4F46E5" },
          { label: "ELIGIBLE", value: summary.totalStudents, sub: "Drive candidates", color: "#2563EB" },
          { label: "APPLICATIONS", value: dataSummary.applicationsCount, sub: `${summary.totalStudents > 0 ? (dataSummary.applicationsCount / summary.totalStudents).toFixed(1) : 0} Apps / Student`, color: "#7C3AED" },
          { label: "PLACED", value: summary.placed, sub: "Confirmed offers", color: "#16A34A" },
          { label: "PLACEMENT RATE", value: `${summary.placementRate.toFixed(1)}%`, sub: `${summary.placed} / ${summary.totalStudents} candidates`, color: "#059669" },
          { label: "AVG SALARY CTC", value: salaryOverview.averagePackage, sub: `Highest: ${salaryOverview.highestPackage}`, color: "#0B3D91" }
        ].map((kpi, idx) => (
          <div
            key={idx}
            style={{
          ...
          >
            <div style={{ fontSize: "10.5px", fontWeight: 700, color: "#64748B", textTransform: "uppercase", letterSpacing: "0.5px" }}>{kpi.label}</div>
